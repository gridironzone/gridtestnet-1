...
# Set node6 token (both for gov min deposit and crisis constant node6)
FEE_TOKEN="utfury"
FROM="\"stake\""
TO="\"$FEE_TOKEN\""
sed -i -e "s/$FROM/$TO/" "$HOME"/.fury/config/genesis.json
...
# Set min-gas-prices (using node6 token)
...
yes $PASSWORD | fury gentx node6 1000000utfury --chain-id $CHAIN_ID
fury collect-gentxs
