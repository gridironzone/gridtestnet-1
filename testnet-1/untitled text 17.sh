...
TOKEN_AMOUNT=${TOKEN_AMOUNT:-"1000000000000utfury"} #1M DYM (1e6dym == 1e12utfury)
STAKING_AMOUNT=${STAKING_AMOUNT:-"670000000000utfury"} #67% is staked (inflation goal)

# Validate fury binary exists
export PATH=$PATH:$HOME/go/bin
if ! command -v fury; then
  make install

  if ! command -v fury; then
    echo "fury binary not found in $PATH"
    exit 1
  fi
fi

# Verify that a genesis file doesn't exists for the fury chain
...
# Create and init fury chain
fury init "$MONIKER_NAME" --chain-id="$CHAIN_ID"
fury tendermint unsafe-reset-all

fury keys add "$KEY_NAME" --keyring-backend test
fury add-genesis-account "$(fury keys show "$KEY_NAME" -a --keyring-backend test)" "$TOKEN_AMOUNT"
fury gentx "$KEY_NAME" "$STAKING_AMOUNT" --chain-id "$CHAIN_ID" --keyring-backend test
