...
# Set sentry4 token (both for gov min deposit and crisis constant sentry4)
FEE_TOKEN="utfury"
FROM="\"stake\""
TO="\"$FEE_TOKEN\""
sed -i -e "s/$FROM/$TO/" "$HOME"/.fury/config/genesis.json
...
# Set min-gas-prices (using sentry4 token)
...
yes $PASSWORD | fury gentx sentry4 1000000utfury --chain-id $CHAIN_ID
fury collect-gentxs
